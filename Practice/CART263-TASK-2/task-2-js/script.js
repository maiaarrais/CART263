...
function createNewDrawingPoint(mouseX, mouseY) {
  ...
  */
     ...
     opacityButton.addEventListener("click", callBack);

     function callBack(){
      if(currentPresets.opacity > 0.1){
        currentPresets.opacity -= 0.1; // decrease opacity by 0.1 
      } else if (currentPresets.opacity <= 0.1){
        currentPresets.opacity = 1.0; 
      }
      let opacityText = opacityButton.querySelector("p");
      opacityText.textContent = currentPresets.opacity.toFixed(1);

      let points = opacityButton.querySelectorAll(".point");
